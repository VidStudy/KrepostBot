...
def edit_category(category_id: int):
    ...
    form.parent.choices = [(c.id, 'RU: {} || UZ: {}'.format(c.get_nested_names(), c.get_nested_names_uz())) for c in all_categories]
...
def create_dish():
    form = DishForm()
    all_categories = dishservice.get_all_categories()
    form.category.choices = [(c.id, 'RU: {} || UZ: {}'.format(c.get_nested_names(), c.get_nested_names_uz())) for c in all_categories]
    if form.validate_on_submit():
